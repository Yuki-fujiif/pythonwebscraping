...
for row in range(2, last_row + 1): #URLリストの総数+1繰り返す。
    ...
    # last_sheetとcurr_sheetの差分比較
    curr_sheet_values = '\n'.join(curr_sheet)
    last_sheet_values = last_sheet.cell(1, 1).value or "" #前回時差分のテキストデータ取得

    diff = difflib.unified_diff([last_sheet_values], [curr_sheet_values])
    diff_list = list(diff)

    # 現在の日付を取得
    current_date = datetime.now(pytz.timezone('Asia/Tokyo')).strftime('%Y-%m-%d,%H:%M')
